...
- `pc_durations_s2.py [--write] <level ...>`: the Season 2 neighbour's station stays into the overlays as PCUseSeconds per visit — the code's (`lap_model_s2.code_stays`) on its CODE levels (203, 208, 209, 211, 212, 213), else the PC videos' bubble spans (docs/PC_LAPS_DETAIL.md, the first lap) less the walk the profile's neighbour takes to each station (the port's idle laps, scratchpad s2_idle_visits.json; since 2026-09-23 walked with the PC's door passes and station runs). Written for every Season 2 level (2026-09-18); the aliases pair PC bubble names with the port's uses in order, a port visit the PC never makes (209's first shoe: the PC does the Taj before the shoes) keeps the mobile length as a leading 0, and a level whose lap is a two-actor handshake (214: the Mother's sit releases his pistol wait) is skipped whole.
