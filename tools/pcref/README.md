...
- `lap_model_s2.py [201 ...]` — the Season 2 lap by code and data, in progress: the level scripts of GameLogic.dll walked symbolically along the untricked path (presence from level.xml's visible objects and the scripts' own hides, shows and switches; IsVariant, isObjectPresent and name compares evaluated; the zero flag followed; event subscriptions and polls handed over) and each step's sequence timed from the data (DoAction `time` or animation frames, hideout enter/leave, message elements instant). Closes 203, 206, 208, 209, 211, 212, 213; the waits (events, polls on Olga and the Mother) and the walks are not timed yet, so the Season 2 stays stay on the video (`pc_durations_s2.py`).
