...
class Hud:
    ...
    def _draw_angry_meter(self, dt):
        ...
        if rott.angry_meter > 0.0 and full is not None and entry is not None:
            ...
            now = self.world.time
            # under the PC profile the Season 1 meter is the PC mercury
            # itself: the rage current over the level's angrytime, clipped
            # (Pawn.tick, pcprofile.s1_rage_percent)
            ...
            if self._angry_rects is None or \
                ...
                # Season 2 gauge is the data's 100 000 (docs/PC_FIDELITY.md §7)
