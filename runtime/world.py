...
class AlerterFSM:
    ...
    def _pc(self):
        w = self.world.woody
        return w is not None and pcprofile.s1_pets(w.nfh2)

    def _pc_rott_in(self):
        rott = self.world.pawns.get('Rottweiler')
        return rott is not None and rott.zone is not None \
            and rott.zone.pid == self.item.zone
