...
- The Season 1 step boundaries around other steps: an ACTION lasts its
  ...
  `Pawn._pc1_close`). ENTER and LEAVE of an object (fcn.00473e20 /
  fcn.00473ea0) are unread — the lap model counts one without a record
  as a step with no timer, two ticks.
- Woody's Season 1 walk: the port walks him on the mobile scene's paths
  — its item positions and depth offsets, its door standing points, a
  climb to every back door and a descent from its twin — at the PC's
  records (17 px a tick along the floor, 6 up and down, 5 and 2
  sneaking); whether game.exe walks him through the neighbour's walk job
  and movers to the clicked object's `woody` hotspot is not read. The
  neighbour's walk is the PC's since 2026-09-26 (docs/PC_FIDELITY.md
  "Season 1 walks": the idle laps within 1.4 s of tools/pcref/
  lap_model.py on every level, docs/PC_LAPS.md), where the mobile's
  paths had put the idle laps 0.3-7.7 s off the model and single legs
  1-3 s (113's angle grinder to the main valve 30.0 against 26.6); the
  model's stations against the bubbles are in docs/PC_LAPS.md: with the
  door pass as one step of its two clips (the row "door transit") E08's
  coffee and E14's kitchen, workshop, pipe and horn stations sit on the
  bubbles, E14's gun and hat 6 and 9 s under them and E13's stations 2-8
  s (the main valve 21.9 against 29-30) — docs/PC_FIDELITY.md "The
  walker's arguments".
- The Season 2 pawns and the Season 1 door rules: `pcprofile.door_warp_early`
  ...
  rule; GameLogic takes the actor out of every room for the pass and sets
  the far room at `<actor>_out` (the row "door transit"), and the catch
  reads that room (`Pawn.pc_room`), not the zone — so the flip moves only
  the zone-keyed hooks' moment. The concurrent pass of 2026-09-26 keys on
  the level's season (`doors_concurrent`), and the Season 2 sweep is
  byte-identical under it; whether the early flip of those eight doors
  matches GameLogic's hooks is not checked.
