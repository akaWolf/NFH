...
- The frame pacer: the timer at `[app+0x50]` (fcn.00402cc0, fcn.00402d30)
  ...
  update is what remains unread; the port keeps its own 12 Hz accumulator. GFXEngine.dll (radare2, the same day) calls `Sleep` once — a 200 ms / 250 ms throttle of its present loop behind a flag (0x1002c1dd–0x1002c20c, the inactive-window pace) — and its GetTickCount/QueryPerformanceCounter pair is the CRT's cookie seed; the renderer has no frame limiter of its own.
- Season 1: whether the position object of the catch is the room or the
  floor strip.
- Season 2: how each catcher's per-level script (the fcn.10011655 table) decides to enter its catch step (fcn.1000eb19's 13 sites: the approach step fcn.1000e601, then `fight`) — the watch modes and the data-side `run` reactions (the failed minigame, Olga's shout) are read
