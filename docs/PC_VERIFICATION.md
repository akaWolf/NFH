...
- Season 1: whether the position object of the catch is the room or the
  floor strip.
- Season 2: who invokes the per-tick event objects' catch handler fcn.1003f086 (slot 2 of vtable 0x100b100c, created by the level tick through fcn.10040f38) and with which actor — the watch modes, the scripted co-actor fights (fcn.1000eb19 after `crash`) and the data-side `run` reactions (the failed minigame, Olga's shout) are read
