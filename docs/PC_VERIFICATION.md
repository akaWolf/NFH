...
- The Season 1 action job's own ticks: Loader.dll stores the record's time
  (above, "the action durations"), and the step's fiber counts it down a
  tick at a time (fcn.00474a20 / fcn.00475850 / fcn.00478120), but the
  ticks the job takes around that count — NFH2's DoActions job is its time
  + 2 (a state-0 update, the count, a state-2 update) — are not read for
  game.exe; the lap model and the stations' `PCUseSeconds` carry the time
  alone (a station of n actions would be 2n ticks longer at NFH2's figure:
  114's hat, five actions, 0.83 s).
