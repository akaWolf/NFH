...
- The Season 1 step boundaries: an ACTION lasts its time + 1 ticks (its
  timer job, carried since 2026-09-26 — "the action durations"), but
  whether the level class's next step starts on the tick its last job
  ends — the order of the level script's pass and the actors' pass in the
  level tick — is not read: ± one tick a step. The start's first-run
  message (the step's +0x18: a 0x14-byte message, vtable 0x4e5448,
...
- The Season 1 chains against Badinfos' runs (2026-09-26): E04 pays its
  ...
  3.8 s and the picture to the dirty oven 5.0 s shorter than the PC's
  ...
  to the vacuum's fire on the PC, 7.0 in the port, whose carpet trigger
...
- `tools/pcref/lap_model.py`, `tools/pcref/trick_branches.py`,
  ...
  action the profile times lasts its time + 1 ticks: the stations and the
  tricked stands, the doors (`DOOR_TICKS`), the shouts (`S1_SHOUT_TICKS`
  92, 25, 45, 26, 26), the pets' clips, Woody's actions and the alerters'
  leave; the lap model 101 33.8 s … 114 171.8 (docs/PC_LAPS.md). The
