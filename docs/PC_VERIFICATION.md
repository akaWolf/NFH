...
- Season 1: whether the position object of the catch is the room or the
  floor strip.
- Season 2: what creates the catch objects (fcn.1003c4e3 in the actor-script table, the reference-less fcn.1003f086) — the watch modes, the scripted co-actor fights (fcn.1000eb19 after `crash`) and the data-side `run` reactions (the failed minigame, Olga's shout) are read
