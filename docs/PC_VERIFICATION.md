...
- Season 1: whether the position object of the catch is the room or the
  floor strip.
- Season 2: which registration gives the catch-class watch entry (slot 2 = fcn.1003f086, vtable 0x100b100c) its mode bit — the walker, the equality, the fire path and the predicate's branches are read — the watch modes, the scripted co-actor fights (fcn.1000eb19 after `crash`) and the data-side `run` reactions (the failed minigame, Olga's shout) are read
