...
- The Season 1 alarm's arrival: a pet's noise sends the neighbour at the
  run (the level class's `noise` case: the `noise` icon, the gait 2 —
  e.g. 111 0x454238-0x4542a9, 114 0x465064-0x4650d5 — and the next
  case), and generic/objects.xml gives him a `search` (25 frames, 24
  ticks); which case plays at the pet and whether it is that `search` is
  not read — the port plays the alerter's RottweilerSurpriseAnimation at
  the remaster's pace there.
