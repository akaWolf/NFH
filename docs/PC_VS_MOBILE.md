...
| | PC | mobile |
|---|---|---|
| Season 1 100 % | the viewer rating, read live off the HUD (E01: 16 → 49 → 72 → 100 for Microwave 16, Binoculars 30, TV 20, Sofa 25 with the tick counter 0/1/2/3; E02: 18, 31, 44, 54, 72, 100 for Beer 18, three 10s, TV 15, Sofa 25 with 0/1/2/2/3/4): each trick pays its TrickScore — the mobile data's values are the PC's — plus 3 per angry tick, the HUD's third counter; the guide's "some scenes require chaining tricks in succession" is those 3s | `GameInfo.CalculateScore` (cs:392-405): FinalTrickScore + CompletedTricksCount × min(CompoundTrickScore, AngryCountTicks), capped 100; a tick is an angry landing while the meter is still above zero (Rottweiler.cs:598-611; 4.23/s decay, 23.6 s window) |
| Season 2 100 % | points, the COLLAPSE! board of all thirteen end screens: 1000 a coin + 3000 for a collapse (once — E10 and E14 fill the gauge twice and still get 3000) + 5000 for the trophy, i.e. every coin (the HUD statue lights at 8/8, not at the overflow: E10 grey after the 299 s fill, gold after the eighth coin at 342) + ⌊500000 / seconds played⌋ (498-500 k on all thirteen: 160 s → 3123, 464 s → 1077; E10: 8000 + 3000 + 5000 + 1398 = 17398), the map counting coins 8/8; the gauge's trick amounts (tools/pcref/amounts.py, levels/pc/*.overlay.json) are near the mobile's: the common coin 25 against 20, the big ones 38-51 against 30-45, the same decay; the gauge decays ~0.4 %/s (measured: E07 17→0 in 46 s, E04 62→51 in 26 s) and may fill ANY number of times — Badinfos' 100 % run fills it twice in 210 (299, 342 s) and 214 (396, 447 s) | int(completed × 90 / total) + 10 iff AngryCountTicks == 1 (cs:413-416): the meter must overflow EXACTLY once; 0.37/s decay (the PC's constant), no pause (Rottweiler.cs:793-796) — GameLogic.dll sums it as 1000 × (coins + lives left) + 5000 for the overflow + 6 000 000 / ticks (docs/PC_ROUTINES.md); the screens cannot tell 3 lives × 1000 from a 3000 |
